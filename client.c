...
int read_request(client_t *client) {
  ...
  offs = 0;
  while (client->psrv->running && (line_end = strstr(buf + offs, "\r\n"))) {
...
int write_to_client(client_t *client, char *buf, int size) {
  ...
  struct pollfd pfd;
	pfd.fd = client->stream_fd;
	pfd.events = POLLIN | POLLHUP | POLLRDNORM;
	pfd.revents = 0;

  if (poll(&pfd, 1, 100) > 0) {
    ERROR_CLIENT(client, "SOcket not active : not all data could be written");
    return -1;
  }

  if (client->stream_fd < 0) {
...
int write_response_ok_header(client_t *client) {
  char *data = (char*)malloc(strlen(RESPONSE_OK_HEADER_TEMPLATE) + 16);
...
int write_response_auth_basic_header(client_t *client) {
  ...
  char *data = (char*)malloc(strlen(RESPONSE_BASIC_AUTH_HEADER_TEMPLATE) + 16 + strlen(realm));
...
void handle_client(client_t *client) {
  ...
  if (result < 0) {
    ...
    cleanup_client(client, client->psrv);
  ...
  while (client->psrv->running) {
    if (pthread_mutex_lock(&(client->psrv->jpeg_mutex))) {
      ERROR_CLIENT(client, "pthread_mutex_lock() failed");
      break;
    }

    while (!client->jpeg_ready) {
      if (pthread_cond_wait(&(client->psrv->jpeg_cond), &(client->psrv->jpeg_mutex))) {
        ERROR_CLIENT(client, "pthread_mutex_wait() failed");
        pthread_mutex_unlock(&(client->psrv->jpeg_mutex));
        break;
      }
    }
    ...
    if (client->psrv->jpeg_size > client->jpeg_tmp_buf_max_size) {
      DEBUG_CLIENT(client, "temporary buffer increased to %d bytes", client->psrv->jpeg_size);
      client->jpeg_tmp_buf_max_size = client->psrv->jpeg_size;
      client->jpeg_tmp_buf = (char*)realloc(client->jpeg_tmp_buf, client->jpeg_tmp_buf_max_size);
    }

    client->jpeg_tmp_buf_size = client->psrv->jpeg_size;
    memcpy(client->jpeg_tmp_buf, client->psrv->jpeg_buf, client->jpeg_tmp_buf_size);

    if (pthread_mutex_unlock(&(client->psrv->jpeg_mutex))) {
